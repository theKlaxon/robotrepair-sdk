...
function SkyModel_CreateDayNightCycle(keys) 
    ...
    result.Date = 1.0
    result.DayLength = 0.0
    result.SynodicMonth = 0.0
    result.Location = ""
    result.NorthAngle = Deg2Rad(QAngle(0.0, 0.0, 0.0).y)
    if vlua.contains(SkyModel_Locations, result.Location) then 
        result = Merge(SkyModel_Locations[result.Location], result)
    else 
        result.Latitude = Deg2Rad(0.0)
        result.Longitude = Deg2Rad(0.0)
        result.Meridian = result.Longitude
        
        local custom_timezone = "UTC-08:00"
